...
// Placeholder route — PR 3 fills in the real subscription screen.
export default function MySubscriptionScreen() {
  const router = useRouter();
  return (
    <SafeAreaView style={styles.safe}>
      <NavBar
        title={COPY.MY_SUBSCRIPTION_TITLE}
        onBackPress={() => router.back()}
      />
      <View style={styles.body}>
        <Text style={styles.placeholder}>{COPY.MY_SUBSCRIPTION_PLACEHOLDER}</Text>
      </View>
    </SafeAreaView>
  );
}
