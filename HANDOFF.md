...
**Written by:** Claude (Opus 4.8) — "My Dogs hero: built a hand-drawn **wooden-frame dog portrait** with a **width-flexible bone name-tag** (`DogPortraitFrame` + `BoneTag`) + 2 polish fixes (removed the CTA drop-shadow; made the avatar row full-bleed). **All UNCOMMITTED on the PR #23 worktree.**" session. (Prior session: built My Dogs Phase 1 → PR #23; coral-CTA WCAG fix → PR #24; `@expo/vector-icons` phantom-dep fix.)
...
**Thread 1 — PR #23 has NEW UNCOMMITTED work; two PRs await iPhone QA + merge (the immediate actionable state):**
- **⚠️ UNCOMMITTED, laptop-only — the dog portrait frame (this session, 2026-06-17).** The wooden-frame dog portrait + flexible bone name-tag built this session lives ONLY as uncommitted working-tree edits in `.worktrees/my-dogs-tab` (on top of `cab328a`). **A fresh clone / cloud instance has NONE of it.** Deep reference **§5A**. The user is QA-ing the latest fix (bone↔ring grommet connection) on their iPhone **right now**. **First job: once their phone QA passes, COMMIT it to the `feature/my-dogs-tab` branch (PR #23)** — files listed in §5A; `git add` them explicitly.
- **PR #23 — My Dogs tab (Phase 1, "Living Scrapbook")** → https://github.com/RohitS199/dog-app-v2/pull/23
  - branch `feature/my-dogs-tab` (committed HEAD `cab328a`), worktree `/Users/rohitsandur/Documents/Projects/dog_app_ui/.worktrees/my-dogs-tab`, **MERGEABLE**, **570/66 at the commit · 578/68 with this session's uncommitted frame work**.
- **PR #24 — coral-CTA WCAG AA fix** → https://github.com/RohitS199/dog-app-v2/pull/24
  - branch `fix/coral-cta-aa-contrast` (HEAD `4ba1f44`), worktree `.worktrees/coral-cta-aa`, **MERGEABLE**, 1 commit, **527 tests / 57 suites green**. Independent of #23 — merges in either order.
- Both branched off `origin/main` (`ebed28c`). **Neither is merged yet** — `origin/main` is still `ebed28c`. The user QAs visual changes on a physical iPhone, then says "merge." Merge with `gh pr merge <n> --merge`.

**Thread 2 — Journey-tab hero + flower garden (parked creative thread, ALL untracked on the laptop):**
- Lives ONLY as untracked files at `/Users/rohitsandur/Documents/Projects/dog_app_ui/`. A fresh clone has **none** of it. Inventory: `preview-journey-hero-option-a-v2.html` (canonical mockup), 3 other `preview-journey-hero-*.html`, `assets/garden/` (**82 MB**: 24 Gemini watercolor flowers + `README.md`), `puplog_garden_scene_prompts.md`. Full reference in **§7**.
- Direction is **locked** (Option A "The Front Yard"); next up is doghouse art + a flower QA pass + 2 product decisions, then brainstorm→spec→plan→RN build. This is the bigger creative arc but is *gated behind* the user's art generation — the two PRs are the thing in flight right now.

**Standing invariants (unchanged):**
- **`origin/main` = `ebed28c`.** **Laptop local `main` = `ac1070c`** — deliberately *diverged*: it has 2 My-Dogs *doc* commits not on origin (`7523c63` spec, `ac1070c` plan), is *behind* origin on the avatar/handoff merges, and carries **~27 uncommitted onboarding-redesign files** + the Thread-2 untracked Journey inventory. **Never pull/rebase/commit-to local `main`.** Branch off `origin/main`, PR back.
...
| Open PRs | **#23** (My Dogs) · **#24** (coral-CTA a11y) — both MERGEABLE, awaiting QA |
| `origin/main` HEAD | `ebed28c` · laptop local `main` = `ac1070c` (diverged on purpose, §4) |
...
- **`origin/main = ebed28c`. TWO open PRs: #23, #24** (both MERGEABLE off `ebed28c`). Other remote branches not in play (ask before touching): `feat/dev-onboarding-bypass`, `feat/envelope-watercolor-icon`, `feature/profile-pr1-foundations`, `feature/profile-redesign-pr2-6`, `fix/onboarding-auth-navigation`, `claude/*`.
...
| What | Path |
| --- | --- |
| ⭐ Canonical mockup | `preview-journey-hero-option-a-v2.html` |
...
**Bottom line:** freshest work — a hand-drawn **wooden-frame dog portrait with a width-flexible bone name-tag** (`DogPortraitFrame` + `BoneTag`) now fronts the My Dogs hero, plus 2 polish fixes (CTA shadow removed, avatar row full-bleed). **It is all UNCOMMITTED on the `.worktrees/my-dogs-tab` worktree (§5A) — committing it to PR #23 after the user's iPhone QA is your first move.** Behind it: the **My Dogs tab is built and production-hardened** (PR #23) and the **coral-CTA WCAG fix is PR #24** — both green, both MERGEABLE, awaiting QA + merge. The **Journey-tab hero** is the parked creative thread (Option A locked, mockup built, 24 flowers wired), gated on Gemini art — resume at §7.7 with brainstorm-lite → spec → plan → TDD. Use the skills, hunt for new ones, make the candidate skills in §14, and mind the hazards (especially H7 `ls` not mtime, H9 spend limit, H11 absolute `cd`, H12 PIL-verify visuals, H14 commit the frame work). 🐾🌼
